            ...
            <div class="card-body">
                <form action="{{ route('admin.footer-grid-one.update', $footer->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input name="url" type="text" class="form-control" value="{{ $footer->url }}">
                        @error('url')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <select name="status" id="" class="form-control">
                            <option {{ $footer->status == 1 ? 'selected' : '' }} value="1">{{ __('admin.Active') }}
                            </option>
                            <option {{ $footer->status == 0 ? 'selected' : '' }} value="0">{{ __('admin.Inactive') }}
                            </option>
                        </select>
                        @error('status')
                            <p class="text-danger">{{ $message }}</p>
